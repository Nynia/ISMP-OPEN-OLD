...
%>
  ...
  <script>
	...
	function tj(){
		...
		if($.trim(userid)==""&&start==""&&end==""){
			window.self.location.href="queryorder";
		}
		if($.trim(userid)!=""&&start==""&&end==""){
			window.self.location.href="queryorder?type=2&userid="+userid;
		}
		if($.trim(userid)==""&&start!=""&&end!=""){
			window.self.location.href="queryorder?type=1&start="+start+"&end="+end;
		}
		if($.trim(userid)!=""&&start!=""&&end!=""){
			window.self.location.href="queryorder?type=3&userid="+userid+"&start="+start+"&end="+end;
		}
...
<body style="min-height:750px;">
    ...
    <div class="am-cf am-padding">
      <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">话单列表</strong> / <small>Order List</small></div>
    </div>
    ...
    <div class="am-g">
      <div class="am-u-sm-12">
        ...
        <a style="" type="button" onclick="tj()" class="am-btn am-btn-primary am-btn-xs">查询话单</a>
          
          <table class="am-table am-table-striped am-table-hover table-main">
            <thead>
              <tr>
                <th class="table-id">ID</th><th class="table-title">用户号码</th><th class="table-type">接入号</th><th class="table-type">短信内容</th><th class="table-date">订购行为</th><th class="table-set">订购时间</th>
              </tr>
          </thead>
          <tbody>
          <%
            ...
            <tr>
              ...
              <td><%=m.get("content")%></td>
              <td><%if("1".equals(m.get("op_type").toString())){out.print("点播");}
              if("2".equals(m.get("op_type").toString())){out.print("包月");}
              if("3".equals(m.get("op_type").toString())){out.print("退订");}
               %></td>
